...
def get_cached_data(ticker):
    try:
        with sqlite3.connect(CACHE_DB) as conn:
            ...
            row = cursor.fetchone()
            # FIX: Explicitly check indices of row tuple safely (row[0] = JSON data text, row[1] = timestamp float)
    ...
    if data:
        ...
        if current_usage >= DAILY_LIMIT:
            ...
            try:
                ...
                # FIX: Safeguard against non-JSON text dumps or HTML blocks from the server
                if response.status_code == 200 and "application/json" in response.headers.get("Content-Type", "").lower():
                    ...
                    # Catch the actual bad HTML response text from Alpha Vantage
